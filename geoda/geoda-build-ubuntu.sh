...
if  [ $OS = 'disco' ] ; then
    ...
    $APT install -y libwebkit2gtk-4.0-dev
    $APT install -y f2c
    $APT install -y libopenblas64-0 libopenblas64-dev
else
...
# Build ATLAS
# source https://math-atlas.sourceforge.net/atlas_install/node6.html
if ! [ -f "atlas3.10.3.tar.bz2" ] ; then
   curl -L -O https://sourceforge.net/projects/math-atlas/files/Stable/3.10.3/atlas3.10.3.tar.bz2
fi

if ! [ -d "ATLAS3.10.3" ] ; then 
